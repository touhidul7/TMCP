...
// ever needed.
export async function listAgentMcpTools({ workspaceId, agentId, scopes = null }) {
  ...
  // Scoped keys only see (and can call) the features their scopes reach.
  const grants = (permsRes.data || []).filter(
    (p) => accountById.has(p.tool_account_id) && scopeAllows(scopes, p.feature_key)
  );
