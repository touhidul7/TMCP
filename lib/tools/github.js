...
function requireRepoName(input) {
  const { name } = input || {};
  if (!name) {
    throw new Error("Missing name in GitHub input parameters");
  }
  return { name };
}
...
async function runGithubTool({ featureKey, input = {}, credentialRecord }) {
  ...
  if (featureKey === "github.create_repository") {
    const { name } = requireRepoName(input);

    if (!hasRealKey) {
      return {
        success: true,
        repository: `sandbox/${name}`,
    ...
    const payload = {
      ...
      ...(typeof input.auto_init === "boolean" ? { auto_init: input.auto_init } : {}),
      ...(input.license_template ? { license_template: input.license_template } : {}),
      ...(input.organization ? { organization: input.organization } : {})
    };

    const ownerPath = input.organization
      ? `orgs/${encodeURIComponent(input.organization)}/repos`
      : "user/repos";

    const repo = await githubJson(await fetch(`https://api.github.com/${ownerPath}`, {
      method: "POST",
    ...
    return {
      success: true,
      repository: repo.full_name || repo.html_url || name,
      repo,
      mode: "authenticated"
    };
  }
