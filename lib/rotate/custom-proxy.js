...
function rotateConfigFor(tool) {
  ...
  return {
    ...
    // Opt-in response cache: identical requests within the TTL are served without pool quota.
    cacheTtlSeconds: Number(cfg.cache_ttl_seconds) > 0 ? Number(cfg.cache_ttl_seconds) : 0
  };
}
...
export async function handleCustomRotate(request, { slug, method = "GET" }) {
  ...
  if (!scopeAllows(scopes, featureKey)) {
    return rotateError(403, `This API key is scoped and does not permit '${featureKey}'.`);
  }
  ...
  if (!minuteCheck.allowed) {
    return rotateError(429, `Rate limit exceeded: per-minute cap of ${perm.perMinuteLimit} reached. Retry shortly.`);
  }
  ...
  // Opt-in response cache: only when the tool config sets cache_ttl_seconds.
  const cacheable = cfg.cacheTtlSeconds > 0 && method !== "HEAD";
  const cacheKey = cacheable
    ? cacheKeyFor({ accountId: account.id, method, path: subPath, query: forwardParams.toString(), body: rawBody })
    : null;
  if (cacheable) {
    ...
    if (hit) {
      logToolCall({
        ...
        featureKey,
        input: { method, path: subPath || "/", cached: true, attempts: 0 },
        status: "SUCCESS",
        latencyMs: Date.now() - startTime
      });
      return new Response(hit.body, {
        status: hit.status,
        headers: { "content-type": hit.contentType || "application/octet-stream", "x-tmcp-cache": "hit" }
      });
    }
  }
  ...
  // Store cacheable successes (skipped when the upstream declares an oversized body).
  if (ok && cacheable) {
