...
async function getWorkspaceUserContext(user, requestedWorkspaceId) {
  ...
  // Guard: check once more in case a concurrent request already created it
  const { data: existingWorkspaceCheck } = await supabaseAdmin
    .from("workspaces")
    .select("id")
    .eq("owner_user_id", userId)
    .limit(1)
  ...
  if (existingWorkspaceCheck) {
    // Another request just created it — use it
    await supabaseAdmin.from("workspace_members").upsert({
      workspace_id: existingWorkspaceCheck.id,
      user_id: userId,
      role: "Owner",
      status: "active"
