export const GATEWAY_ENDPOINTS = [
  ...
  },
  {
    ...
    description: "Spec-compliant Model Context Protocol server: initialize, tools/list, and tools/call over JSON-RPC 2.0. Connect any MCP client with this URL and a Bearer agent key."
  },
  {
    method: "GET",
    path: "/api/gateway/approvals/{approval_id}",
    name: "Approval Status",
    description: "Poll a pending approval-gated call. Once an administrator approves it, the executed result is returned here."
  },
  {
    method: "POST",
    path: "/api/gateway/keys/mint",
    name: "Mint Scoped Key",
    description: "Create a scoped, expiring child API key restricted to specific feature keys (e.g. [\"serper.search\", \"gmail.*\"]). A scoped key can only mint narrower keys."
  },
  {
    method: "GET",
    path: "/api/openapi",
    name: "OpenAPI Specification",
    description: "Machine-readable OpenAPI 3.1 description of the agent-facing API for SDK generators and API tooling. No auth required."
  }
];
...
// Examples for the Apify-compatible transparent proxy. The request is identical to a direct Apify
// API call, except the base URL is TMCP's and the bearer token is a TMCP agent API key.
export function buildApifyRotateSnippets({ baseUrl = "https://your-domain.com", apiKey = "mcp_live_xxxxxxxxxxxx", basePath = "/api/apify/v2", path = "/acts" } = {}) {
  const url = `${baseUrl}${basePath}${path}`;
  return {
    curl: `# Any Apify endpoint works — just swap the base URL and use your TMCP key as the token.
curl "${url}" \\
  -H "Authorization: Bearer ${apiKey}"`,
    javascript: `// Drop-in for https://api.apify.com/v2 — same paths/methods, TMCP base URL + TMCP token.
...
// Examples for the Serper-compatible transparent proxy. The request is identical to a direct Serper
// API call, except the base URL is TMCP's and the X-API-KEY is a TMCP agent API key.
export function buildSerperRotateSnippets({ baseUrl = "https://your-domain.com", apiKey = "mcp_live_xxxxxxxxxxxx", basePath = "/api/serper", path = "/search" } = {}) {
  const url = `${baseUrl}${basePath}${path}`;
  const scrapeUrl = `${baseUrl}${basePath}/scrape`;
  const payload = { q: "apple inc" };
  const json = JSON.stringify(payload, null, 2);
  return {
    curl: `# Search API — drop-in for https://google.serper.dev (swap base URL, TMCP key as X-API-KEY):
curl -X POST "${url}" \\
  -H "X-API-KEY: ${apiKey}" \\
  -H "Content-Type: application/json" \\
  -d '${json}'

# Scrape API — drop-in for https://scrape.serper.dev (same key pool):
curl -X POST "${scrapeUrl}" \\
  -H "X-API-KEY: ${apiKey}" \\
  -H "Content-Type: application/json" \\
  -d '{"url": "https://example.com"}'`,
    javascript: `// Search API — drop-in for https://google.serper.dev (same paths/methods, TMCP base URL + key).
const res = await fetch("${url}", {
  method: "POST",
...
// Scrape API — drop-in for https://scrape.serper.dev (same key pool).
const scraped = await fetch("${scrapeUrl}", {
  method: "POST",
  headers: { "X-API-KEY": "${apiKey}", "Content-Type": "application/json" },
...
# Search API — drop-in for https://google.serper.dev
res = requests.post(
    "${url}",
    headers={"X-API-KEY": "${apiKey}", "Content-Type": "application/json"},
    json=${json.replace(/\n/g, "\n    ")},
...
// Rotate tools are NOT invoked through /api/gateway/execute — each has its own dedicated base URL.
// This map drives correct, non-misleading docs/examples for them everywhere they're surfaced.
export const ROTATE_TOOL_META = {
  "gemini-rotate":     { kind: "openai",   basePath: "/api/gemini/v1",     model: "gemini-2.5-flash" },
  "openrouter-rotate": { kind: "openai",   basePath: "/api/openrouter/v1", model: "openai/gpt-4o-mini" },
  "scrapedo-rotate":   { kind: "scrapedo", basePath: "/api/scrapedo" },
  "apify-rotate":      { kind: "apify",    basePath: "/api/apify/v2" },
  "serper-rotate":     { kind: "serper",   basePath: "/api/serper" }
};

export function isRotateSlug(slug) {
  return Boolean(ROTATE_TOOL_META[slug]);
}

// Build a self-contained doc block for a rotate tool: how it is actually called (its own base URL,
// not /api/gateway/execute), a correct example request, and copy-pasteable code snippets.
export function buildRotateToolDoc({ baseUrl = "https://your-domain.com", apiKey = "mcp_live_xxxxxxxxxxxx", slug } = {}) {
  const meta = ROTATE_TOOL_META[slug];
  if (!meta) return null;

  if (meta.kind === "scrapedo") {
    const exampleTarget = "https://example.com";
    return {
      invocation: "dedicated_base_url",
      base_url: `${baseUrl}${meta.basePath}`,
      usage_note: "Scrape.do-compatible proxy — do NOT call /api/gateway/execute. Send a request to the base URL with the TMCP agent key as the 'token' query parameter (or an 'Authorization: Bearer' header). 'url' is required; every other Scrape.do parameter (render, super, geoCode, …) is forwarded unchanged. GET and POST are supported.",
      example_request: {
        method: "GET",
        url: `${baseUrl}${meta.basePath}?token=${apiKey}&url=${encodeURIComponent(exampleTarget)}&render=true`
      },
      examples: buildScrapeDoRotateSnippets({ baseUrl, apiKey, basePath: meta.basePath, targetUrl: exampleTarget })
    };
  }

  if (meta.kind === "apify") {
    const examplePath = "/acts";
    return {
      invocation: "dedicated_base_url",
      base_url: `${baseUrl}${meta.basePath}`,
      usage_note: "Apify-compatible transparent proxy — do NOT call /api/gateway/execute. Use this base URL in place of https://api.apify.com/v2 and the TMCP agent key in place of your Apify token (bearer header or 'token' query param). Any Apify endpoint, method, path, query, and body is forwarded unchanged. Failover happens on 401/402/403/408/429.",
      example_request: {
        method: "GET",
        url: `${baseUrl}${meta.basePath}${examplePath}`,
        headers: { Authorization: `Bearer ${apiKey}` }
      },
      examples: buildApifyRotateSnippets({ baseUrl, apiKey, basePath: meta.basePath, path: examplePath })
    };
  }

  if (meta.kind === "serper") {
    const examplePath = "/search";
    return {
      invocation: "dedicated_base_url",
      base_url: `${baseUrl}${meta.basePath}`,
      usage_note: "Serper-compatible transparent proxy — do NOT call /api/gateway/execute. Put the TMCP agent key in the X-API-KEY header in place of your Serper key (one rotating pool serves both Serper products). Search API: use this base URL in place of https://google.serper.dev (e.g. POST /api/serper/search, /api/serper/images, …). Scrape API: use {base}/api/serper/scrape in place of https://scrape.serper.dev (POST a { url } body). Method, query, and body are forwarded unchanged; failover happens on 401/402/403/429.",
      example_request: {
        method: "POST",
        url: `${baseUrl}${meta.basePath}${examplePath}`,
        headers: { "X-API-KEY": apiKey, "Content-Type": "application/json" },
      ...
      examples: buildSerperRotateSnippets({ baseUrl, apiKey, basePath: meta.basePath, path: examplePath })
    };
  }

  // OpenAI-compatible rotate tools (Gemini, OpenRouter).
  return {
    invocation: "dedicated_base_url",
    base_url: `${baseUrl}${meta.basePath}`,
    usage_note: "OpenAI-compatible — do NOT call /api/gateway/execute. Set this base URL in any OpenAI client/SDK and use the TMCP agent key as the bearer token. Endpoints: /chat/completions, /responses, /embeddings, /models.",
    example_request: {
      method: "POST",
      url: `${baseUrl}${meta.basePath}/chat/completions`,
      headers: { Authorization: `Bearer ${apiKey}`, "Content-Type": "application/json" },
      body: { model: meta.model, messages: [{ role: "user", content: "Hello" }] }
    },
    examples: buildRotateSnippets({ baseUrl, apiKey, basePath: meta.basePath, model: meta.model })
  };
}
