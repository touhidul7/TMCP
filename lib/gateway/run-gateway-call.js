...
import { checkAgentToolPermission, countDailyUsage, checkMinuteRateLimit } from "../permissions/check-agent-tool-permission";
...
export async function runGatewayCall({
  ...
  // Scoped keys narrow access before the permission matrix is even consulted.
  if (!scopeAllows(scopes, featureKey)) {
    const reason = `Access denied: this API key is scoped and does not permit '${featureKey}'.`;
  ...
  // Per-minute burst limit (only queried when the permission row sets one).
  const minuteCheck = await checkMinuteRateLimit({
    agentId,
    apiKeyId,
    featureKey,
    perMinuteLimit: permCheck.perMinuteLimit
  });
  if (!minuteCheck.allowed) {
    const limitErr = `Rate limit exceeded: Per-minute cap of ${permCheck.perMinuteLimit} reached. Retry shortly.`;
    logToolCall({ ...logBase, status: "DENIED", error: limitErr, latencyMs: latency() });
    return { kind: "rate_limited", error: limitErr, perMinuteLimit: permCheck.perMinuteLimit };
  }
