...
export async function handleScrapeDoRotate(request, { method = "GET" } = {}) {
  ...
  if (!scopeAllows(scopes, FEATURE_KEY)) {
    return scrapeError(403, `This API key is scoped and does not permit '${FEATURE_KEY}'.`);
  }
  ...
  if (!minuteCheck.allowed) {
    return scrapeError(429, `Rate limit exceeded: per-minute cap of ${perm.perMinuteLimit} reached. Retry shortly.`);
  }
