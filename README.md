...
- Outgoing email (invites, approval notifications, job deliveries) is configured entirely by environment: `RESEND_API_KEY` + `RESEND_DOMAIN` (sender is `tmcp@{RESEND_DOMAIN}`); with either unset, approval notifications and job deliveries are skipped gracefully, while user invitations fail with an explicit error (the invite record is rolled back so it can be retried once email is configured). Invite sends also surface Resend delivery errors instead of reporting success.
