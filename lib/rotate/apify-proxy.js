...
export async function handleApifyRotate(request, { method = "GET" } = {}) {
  ...
  try {
    ...
    return apifyError(500, accErr.message || "Failed to resolve rotate account");
  }
  if (!account) {
  ...
  if (!scopeAllows(scopes, FEATURE_KEY)) {
    return apifyError(403, `This API key is scoped and does not permit '${FEATURE_KEY}'.`);
  }
  ...
  if (!minuteCheck.allowed) {
    return apifyError(429, `Rate limit exceeded: per-minute cap of ${perm.perMinuteLimit} reached. Retry shortly.`);
  }
