...
export function MockStoreProvider({ children }) {
  ...
  const fetchLiveDatabaseData = async (sessionUser) => {
    try {
      ...
      const builtInList = INITIAL_BUILTIN_TOOLS.map(t => ({ ...t, workspace_id: wsId }));
      setTools([...builtInList, ...(dbTools || [])]);

      // Fetch Tool Features
      const { data: dbFeatures } = await supabase.from("tool_features").select("*");
      setFeatures([...INITIAL_FEATURES, ...(dbFeatures || [])]);
  ...
  const inviteUser = async (name, email, role) => {
    ...
    if (useLiveDb) {
      const { error } = await supabase.from("workspace_invitations").insert({
        ...
        email,
        role,
        token_hash: "dummy_invitation_hash",
        status: "pending"
      });
  ...
  const addToolAccount = async (toolId, label, credentials) => {
    ...
    if (useLiveDb) {
      const res = await fetch("/api/tool-accounts/add", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ toolId, label, credentials })
      });
      const data = await res.json();
      if (!data.success) return { error: data.error };
      fetchLiveDatabaseData(user);
  ...
  const switchWorkspace = (workspaceId) => {
    ...
    if (useLiveDb && user) {
      fetchLiveDatabaseData({ ...user, id: user.id, email: user.email, user_metadata: user.user_metadata });
    } else {
